...
import com.example.tried.auth.reports.trust_funds_date_to_date.TransactionsItem;
...
@Service
public class TrustFundDateToDateSummary {
    ...
    public void trustFundSummaryReport(TrustFundSummaryDateToDatePaymentMode trustFundSummary, HttpServletResponse OutputResponse,
                                      String church_code,String church_name,String personnel_name,
                                       String personnel_number,String conference_name,String district_name) throws IOException {
        ...
        addHeaderData(table, church_name, conference_name, district_name);
    ...
    private void addHeaderData(Table table, String church_name, String conference_name, String district_name) {
    ...
    private void addHorizontalLine(Table invoice_header, TrustFundSummaryDateToDatePaymentMode trustFundSummary){
        Border b2 = new SolidBorder(new DeviceRgb(53, 133, 146), 4);

        String Church_Name = trustFundSummary.getAuthentication().getInstitutionName();
        LocalDate date = LocalDate.now();
        String startDate = trustFundSummary.getPayload().getStartDate();
        String endDate = trustFundSummary.getPayload().getEndDate();

        invoice_header.setBorder(Border.NO_BORDER);
        invoice_header.addCell(new Cell().add("Full Trust Fund Summary Report "+ Church_Name + " from " + startDate + " to " + endDate).setBorder(Border.NO_BORDER))
                .setFontSize(9)
                .setFont(bolden)
                .setWidthPercent(100)
                .setBold()
    ...
    private void getTrustFundSummaryReport(Table summaryReport, TrustFundSummaryDateToDatePaymentMode trustFundSummary) throws JsonProcessingException {

        // System.out.println("Trust Fund Summary Report: " + HelperUtility.toJSON(trustFundSummary));

        TrustFundSummaryDateToDatePaymentModeResponse localChurchTrustFund = personnelApi.getTrustFundSummaryDateByDateWithPaymentMode(trustFundSummary);

        System.out.println("Local Church Trust Fund Summary Response: " + HelperUtility.toJSON(localChurchTrustFund));

        List<com.example.tried.auth.reports.payment_mode.date_to_date.TransactionsItem> transactions = localChurchTrustFund
                .getMdpayload().getTransactions();
